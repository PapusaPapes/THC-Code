...
namespace PlayerCoder
{
    ...
    // Team: Fighter / Rogue / Cleric
    // Items: 3 Ether, 1 Silence Remedy, 5 Poison Remedy, 3 Petrify Remedy, 3 Full Remedy = 70g
    // Strategy:
    // Fighter stacks Brave, Covers the Rogue, and Resurrects fallen allies.
    ...
    public static class MyAI
    {
        ...
        // Rogue is most important to cleanse, it's the control engine
        ...
        private static void ControlFighter(Hero actor)
        {
            ...
            // Ctrl & Sustain: focus Alchemist, save MP for Resurrection
            if (IsCtrlAndSustainLike())
            {
                ...
                if (alchemist != null)
                {
                    ...
                    if (actor.mana >= 40 && Act(actor, Ability.QuickHit, alchemist)) return;
        ...
        private static void ControlRogue(Hero actor)
        {
            ...
            // Item Crafter: Steal their items, Silence Alchemist, poison Rogues
        ...
        private static void ControlCleric(Hero actor)
        {
            ...
            // Ctrl & Sustain: heal Rogue with QuickHeal, attack Alchemist
            if (IsCtrlAndSustainLike())
            {
                Hero rogue = FindLivingAlly(HeroJobClass.Rogue);
                if (rogue != null && HpRatio(rogue) <= HpLight &&
                    Act(actor, Ability.QuickHeal, rogue)) return;
        ...
        private static bool ApplyBuffs(Hero actor)
        {
            ...
            Hero rogue = FindLivingAlly(HeroJobClass.Rogue);
            if (rogue != null && !HasStatus(rogue, StatusEffect.Haste) &&
        ...
        {
            // Silenced Rogue can't control, clear it immediately
            Hero rogue = FindLivingAlly(HeroJobClass.Rogue);
            if (rogue != null && HasStatus(rogue, StatusEffect.Silence))
            {
                if (Act(actor, Ability.SilenceRemedy, rogue)) return true;
                if (Act(actor, Ability.FullRemedy,    rogue)) return true;
            }
        ...
        {
            // In Ctrl & Sustain only the Cleric needs mana
            if (IsCtrlAndSustainLike())
            {
                Hero cleric = FindLivingAlly(HeroJobClass.Cleric);
                if (cleric != null && MpRatio(cleric) <= threshold)
                    return Act(actor, Ability.Ether, cleric);
                return false;
            }
        ...
        // 2 Alchemists + Monk, both Alchemists revive each other
